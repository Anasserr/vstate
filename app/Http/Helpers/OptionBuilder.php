...
class OptionBuilder
{
    ...
    public function setApns()
    {
        $header = [ 'apns-priority' => 5 ] ;
        $headerFinal = array('headers' => $header ) ;
        $category = array('category' => 'NEW_MESSAGE_CATEGORY') ;
        $apsFinal = array('aps' =>  $category );
        $payloadFinal = array('payload' =>  $apsFinal );
        $apns = array($headerFinal ,$payloadFinal ) ;
        $this->apns = $apns ;
        return $this;
    }
    ...
    {
        return  new \App\Helpers\Options($this);
    }
}
