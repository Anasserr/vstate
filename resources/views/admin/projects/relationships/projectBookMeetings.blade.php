...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-projectBookMeetings">
                <thead>
                    <tr>
                        <th width="10">
                        ...
                        <tr data-entry-id="{{ $bookMeeting->id }}">
                            ...
                            <td>
                                @can('book_meeting_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.book-meetings.show', $bookMeeting->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('book_meeting_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.book-meetings.edit', $bookMeeting->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('book_meeting_delete')
                                    <form action="{{ route('admin.book-meetings.destroy', $bookMeeting->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-projectBookMeetings:not(.ajaxTable)').DataTable({ buttons: dtButtons })
