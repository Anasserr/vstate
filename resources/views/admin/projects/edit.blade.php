    ...
    <div class="card">
        ...
        <div class="card-body">
            <form class="row g-3 fv-plugins-bootstrap5 fv-plugins-framework" method="POST"
                action="{{ route('admin.projects.update', [$project->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                ...
                <div class="form-group">
                    <div class="form-check {{ $errors->has('active') ? 'is-invalid' : '' }}">
                        ...
                        <input class="form-check-input" type="checkbox" name="active" id="active" value="1"
                            {{ $project->active || old('active', 0) === 1 ? 'checked' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status"
                        ...
                        @foreach (App\Models\Project::STATUS_SELECT as $key => $label)
                            <option value="{{ $key }}"
                                {{ old('status', $project->status) === (string) $key ? 'selected' : '' }}>
                                {{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('lat') ? 'is-invalid' : '' }}" type="text" name="lat"
                        id="lat" value="{{ old('lat', $project->lat) }}">
                    @if ($errors->has('lat'))
                        <div class="invalid-feedback">
                            {{ $errors->first('lat') }}
                        </div>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('lang') ? 'is-invalid' : '' }}" type="text"
                        name="lang" id="lang" value="{{ old('lang', $project->lang) }}">
                    @if ($errors->has('lang'))
                        <div class="invalid-feedback">
                            {{ $errors->first('lang') }}
                        </div>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('location_link') ? 'is-invalid' : '' }}" name="location_link"
                        id="location_link">{{ old('location_link', $project->location_link) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('addresse') ? 'is-invalid' : '' }}" type="text"
                        name="addresse" id="addresse" value="{{ old('addresse', $project->addresse) }}">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('city') ? 'is-invalid' : '' }}" name="city_id"
                        ...
                        @foreach ($cities as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('city_id') ? old('city_id') : $project->city->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('city'))
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('nearbies') ? 'is-invalid' : '' }}"
                        name="nearbies[]" id="nearbies" multiple>
                        @foreach ($nearbies as $id => $nearby)
                            <option value="{{ $id }}"
                                {{ in_array($id, old('nearbies', [])) || $project->nearbies->contains($id) ? 'selected' : '' }}>
                                {{ $nearby }}</option>
                        @endforeach
                    </select>
                    ...
                    @foreach (App\Models\Project::FEATURED_RADIO as $key => $label)
                        <div class="form-check {{ $errors->has('featured') ? 'is-invalid' : '' }}">
                            <input class="form-check-input" type="radio" id="featured_{{ $key }}"
                                ...
                                {{ old('featured', $project->featured) === (string) $key ? 'checked' : '' }}>
                            <label class="form-check-label"
                                for="featured_{{ $key }}">{{ $label }}</label>
                        </div>
                    @endforeach
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('project_type') ? 'is-invalid' : '' }}"
                        name="project_type_id" id="project_type_id">
                        @foreach ($project_types as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('project_type_id') ? old('project_type_id') : $project->project_type->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('plan_title') ? 'is-invalid' : '' }}" type="text"
                        name="plan_title" id="plan_title" value="{{ old('plan_title', $project->plan_title) }}">
                ...
                <div class="form-group">
                    ...
                    <div class="needsclick dropzone {{ $errors->has('plan_image') ? 'is-invalid' : '' }}"
                        id="plan_image-dropzone">
                    </div>
                ...
                <div class="form-group">
                    <label
                        for="construction_updates_images">{{ trans('cruds.project.fields.construction_updates_images') }}</label>
                    <div class="needsclick dropzone {{ $errors->has('construction_updates_images') ? 'is-invalid' : '' }}"
                        id="construction_updates_images-dropzone">
                    </div>
                    ...
                    @endif
                    <span
                        class="help-block">{{ trans('cruds.project.fields.construction_updates_images_helper') }}</span>
                </div>
                <div class="form-group">
                    <label
                        for="construction_updates_videos">{{ trans('cruds.project.fields.construction_updates_videos') }}</label>
                    <div class="needsclick dropzone {{ $errors->has('construction_updates_videos') ? 'is-invalid' : '' }}"
                        id="construction_updates_videos-dropzone">
                    </div>
                    ...
                    @endif
                    <span
                        class="help-block">{{ trans('cruds.project.fields.construction_updates_videos_helper') }}</span>
                </div>
                <div class="form-group">
                    <label for="second_title">{{ trans('cruds.project.fields.second_title') }}</label>
                    <input class="form-control {{ $errors->has('second_title') ? 'is-invalid' : '' }}" type="text"
                        name="second_title" id="second_title"
                        value="{{ old('second_title', $project->second_title) }}">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control ckeditor {{ $errors->has('second_description') ? 'is-invalid' : '' }}"
                        name="second_description" id="second_description">{!! old('second_description', $project->second_description) !!}</textarea>
                ...
                <div class="form-group">
                    ...
                    <div class="needsclick dropzone {{ $errors->has('second_image') ? 'is-invalid' : '' }}"
                        id="second_image-dropzone">
                    </div>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('plan_description') ? 'is-invalid' : '' }}"
                        type="text" name="plan_description" id="plan_description"
                        value="{{ old('plan_description', $project->plan_description) }}">
                ...
                <div class="form-group">
                    ...
                    <div class="needsclick dropzone {{ $errors->has('banners') ? 'is-invalid' : '' }}"
                        id="banners-dropzone">
                    </div>
...
@section('scripts')
    ...
    <script>
        ...
        Dropzone.options.constructionUpdatesImagesDropzone = {
            ...
            success: function(file, response) {
                $('form').append('<input type="hidden" name="construction_updates_images[]" value="' + response
                    .name + '">')
            ...
            init: function() {
                @if (isset($project) && $project->construction_updates_images)
                    ...
                    for (var i in files) {
                        ...
                        this.options.thumbnail.call(this, file, file.preview ?? file.preview_url)
                        file.previewElement.classList.add('dz-complete')
                        $('form').append('<input type="hidden" name="construction_updates_images[]" value="' + file
                            .file_name + '">')
                    }
                @endif
            },
    ...
    <script>
        ...
        Dropzone.options.constructionUpdatesVideosDropzone = {
            ...
            success: function(file, response) {
                $('form').append('<input type="hidden" name="construction_updates_videos[]" value="' + response
                    .name + '">')
            ...
            init: function() {
                @if (isset($project) && $project->construction_updates_videos)
                    ...
                    for (var i in files) {
                        ...
                        this.options.addedfile.call(this, file)
                        file.previewElement.classList.add('dz-complete')
                        $('form').append('<input type="hidden" name="construction_updates_videos[]" value="' + file
                            .file_name + '">')
                    }
                @endif
            },
