        ...
        <form method="POST" action="{{ route("admin.roles.update", [$role->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('show_in_website') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="show_in_website" id="show_in_website" value="1" {{ $role->show_in_website || old('show_in_website', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="show_in_website">{{ trans('cruds.role.fields.show_in_website') }}</label>
                </div>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Role::ACTIVE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('active') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="active_{{ $key }}" name="active" value="{{ $key }}" {{ old('active', $role->active) === (string) $key ? 'checked' : '' }}>
