...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.eventDiscussion.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.event-discussions.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('discussion') ? 'is-invalid' : '' }}" name="discussion" id="discussion">{{ old('discussion') }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('replay_discussion') ? 'is-invalid' : '' }}" name="replay_discussion_id" id="replay_discussion_id">
                    @foreach($replay_discussions as $id => $entry)
                        <option value="{{ $id }}" {{ old('replay_discussion_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
