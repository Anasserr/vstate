...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.realEstateApplication.title_singular') }}
        </div>

        <div class="card-body">
            <form class="row g-3 fv-plugins-bootstrap5 fv-plugins-framework" method="POST"
                action="{{ route('admin.real-estate-applications.store') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('max_price') ? 'is-invalid' : '' }}" type="number"
                        name="max_price" id="max_price" value="{{ old('max_price', '') }}" step="0.01">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('min_price') ? 'is-invalid' : '' }}" type="number"
                        name="min_price" id="min_price" value="{{ old('min_price', '') }}" step="0.01">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('deliver_year') ? 'is-invalid' : '' }}" type="text"
                        name="deliver_year" id="deliver_year" value="{{ old('deliver_year', '') }}">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('number_of_rooms') ? 'is-invalid' : '' }}" type="text"
                        name="number_of_rooms" id="number_of_rooms" value="{{ old('number_of_rooms', '') }}">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('user_phone') ? 'is-invalid' : '' }}" type="text"
                        name="user_phone" id="user_phone" value="{{ old('user_phone', '') }}">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('time_of_call') ? 'is-invalid' : '' }}" type="text"
                        name="time_of_call" id="time_of_call" value="{{ old('time_of_call', '') }}">
                ...
                <div class="form-group">
                    ...
                    <select
                        ...
                        @foreach ($listings_available_for_mortgages as $id => $listings_available_for_mortgage)
                            <option value="{{ $id }}"
                                {{ in_array($id, old('listings_available_for_mortgages', [])) ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('purpose_of_request') ? 'is-invalid' : '' }}"
                        ...
                        @foreach (App\Models\RealEstateApplication::PURPOSE_OF_REQUEST_SELECT as $key => $label)
                            <option value="{{ $key }}"
                                {{ old('purpose_of_request', '') === (string) $key ? 'selected' : '' }}>
                                {{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('payment_methods') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($payment_methods as $id => $payment_method)
                            <option value="{{ $id }}"
                                {{ in_array($id, old('payment_methods', [])) ? 'selected' : '' }}>{{ $payment_method }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('types') ? 'is-invalid' : '' }}" name="types[]"
                        ...
                        @foreach ($types as $id => $type)
                            <option value="{{ $id }}"
                                {{ in_array($id, old('types', [])) ? 'selected' : '' }}>{{ $type }}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('types'))
                        <div class="invalid-feedback">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('views') ? 'is-invalid' : '' }}" name="views[]"
                        ...
                        @foreach ($views as $id => $view)
                            <option value="{{ $id }}"
                                {{ in_array($id, old('views', [])) ? 'selected' : '' }}>{{ $view }}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('views'))
                        <div class="invalid-feedback">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('finish_types') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($finish_types as $id => $finish_type)
                            <option value="{{ $id }}"
                                {{ in_array($id, old('finish_types', [])) ? 'selected' : '' }}>{{ $finish_type }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('min_area') ? 'is-invalid' : '' }}" type="text"
                        name="min_area" id="min_area" value="{{ old('min_area', '') }}">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('max_area') ? 'is-invalid' : '' }}" type="text"
                        name="max_area" id="max_area" value="{{ old('max_area', '') }}">
