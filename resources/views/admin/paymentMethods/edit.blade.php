    ...
    <div class="card">
        ...
        <div class="card-body">
            <form class="row g-3 fv-plugins-bootstrap5 fv-plugins-framework" method="POST"
                action="{{ route('admin.payment-methods.update', [$paymentMethod->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="form-group">
                    <label class="required" for="title">{{ trans('cruds.paymentMethod.fields.title') }}</label>
                    <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text"
                        name="title" id="title" value="{{ old('title', $paymentMethod->title) }}" required>
                    @if ($errors->has('title'))
                ...
                <div class="form-group">
                    <div class="form-check {{ $errors->has('active') ? 'is-invalid' : '' }}">
                        ...
                        <input class="form-check-input" type="checkbox" name="active" id="active" value="1"
                            {{ $paymentMethod->active || old('active', 0) === 1 ? 'checked' : '' }}>
