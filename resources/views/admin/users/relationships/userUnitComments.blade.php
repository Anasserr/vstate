...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-userUnitComments">
                <thead>
                    <tr>
                        <th width="10">
                            ...
                            <td>
                                @can('unit_comment_show')
                                    <a class="btn btn-xs btn-primary"
                                        href="{{ route('admin.unit-comments.show', $unitComment->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan
                                ...
                                @can('unit_comment_edit')
                                    <a class="btn btn-xs btn-info"
                                        href="{{ route('admin.unit-comments.edit', $unitComment->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan
                                ...
                                @can('unit_comment_delete')
                                    <form class="row g-3 fv-plugins-bootstrap5 fv-plugins-framework"
                                        action="{{ route('admin.unit-comments.destroy', $unitComment->id) }}"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-userUnitComments:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
