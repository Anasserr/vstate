...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-portTypeTransportationTypes">
                <thead>
                    <tr>
                        <th width="10">
                    ...
                    @foreach($transportationTypes as $key => $transportationType)
                        <tr data-entry-id="{{ $transportationType->id }}">
                            ...
                            <td>
                                ...
                                <input type="checkbox" disabled="disabled" {{ $transportationType->active ? 'checked' : '' }}>
                            </td>
                            <td>
                                @can('transportation_type_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.transportation-types.show', $transportationType->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('transportation_type_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.transportation-types.edit', $transportationType->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('transportation_type_delete')
                                    <form action="{{ route('admin.transportation-types.destroy', $transportationType->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-portTypeTransportationTypes:not(.ajaxTable)').DataTable({ buttons: dtButtons })
