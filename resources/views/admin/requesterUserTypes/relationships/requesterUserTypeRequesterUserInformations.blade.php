...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table
                class=" table table-bordered table-striped table-hover datatable datatable-requesterUserTypeRequesterUserInformations">
                <thead>
                    <tr>
                        <th width="10">
                            ...
                            <td>
                                @can('requester_user_information_show')
                                    <a class="btn btn-xs btn-primary"
                                        href="{{ route('admin.requester-user-informations.show', $requesterUserInformation->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan
                                ...
                                @can('requester_user_information_edit')
                                    <a class="btn btn-xs btn-info"
                                        href="{{ route('admin.requester-user-informations.edit', $requesterUserInformation->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan
                                ...
                                @can('requester_user_information_delete')
                                    <form class="row g-3 fv-plugins-bootstrap5 fv-plugins-framework"
                                        action="{{ route('admin.requester-user-informations.destroy', $requesterUserInformation->id) }}"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-requesterUserTypeRequesterUserInformations:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
