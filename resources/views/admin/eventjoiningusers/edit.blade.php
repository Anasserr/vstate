    ...
    <div class="card">
        ...
        <div class="card-body">
            <form class="row g-3 fv-plugins-bootstrap5 fv-plugins-framework" method="POST"
                action="{{ route('admin.eventjoiningusers.update', [$eventjoininguser->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="form-group">
                    <div class="form-check {{ $errors->has('block') ? 'is-invalid' : '' }}">
                        ...
                        <input class="form-check-input" type="checkbox" name="block" id="block" value="1"
                            {{ $eventjoininguser->block || old('block', 0) === 1 ? 'checked' : '' }}>
                        <label class="form-check-label"
                            for="block">{{ trans('cruds.eventjoininguser.fields.block') }}</label>
                    </div>
                    @if ($errors->has('block'))
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('event') ? 'is-invalid' : '' }}" name="event_id"
                        ...
                        @foreach ($events as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('event_id') ? old('event_id') : $eventjoininguser->event->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('event'))
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id"
                        ...
                        @foreach ($users as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('user_id') ? old('user_id') : $eventjoininguser->user->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('user'))
                ...
                <div class="form-group">
                    <label class="required"
                        for="event_status_id">{{ trans('cruds.eventjoininguser.fields.event_status') }}</label>
                    <select class="form-control select2 {{ $errors->has('event_status') ? 'is-invalid' : '' }}"
                        name="event_status_id" id="event_status_id" required>
                        @foreach ($event_statuses as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('event_status_id') ? old('event_status_id') : $eventjoininguser->event_status->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
