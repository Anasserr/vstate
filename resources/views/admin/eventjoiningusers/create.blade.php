...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.eventjoiningusers.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <div class="form-check {{ $errors->has('block') ? 'is-invalid' : '' }}">
                    <input type="hidden" name="block" value="0">
                    <input class="form-check-input" type="checkbox" name="block" id="block" value="1" {{ old('block', 0) == 1 || old('block') === null ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('event_status') ? 'is-invalid' : '' }}" name="event_status_id" id="event_status_id" required>
                    @foreach($event_statuses as $id => $entry)
                        <option value="{{ $id }}" {{ old('event_status_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
