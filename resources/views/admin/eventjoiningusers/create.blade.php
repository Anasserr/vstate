...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.eventjoininguser.title_singular') }}
        </div>

        <div class="card-body">
            <form class="row g-3 fv-plugins-bootstrap5 fv-plugins-framework" method="POST"
                action="{{ route('admin.eventjoiningusers.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="form-group">
                    <div class="form-check {{ $errors->has('block') ? 'is-invalid' : '' }}">
                        ...
                        <input class="form-check-input" type="checkbox" name="block" id="block" value="1"
                            {{ old('block', 0) == 1 || old('block') === null ? 'checked' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('event_status') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($event_statuses as $id => $entry)
                            <option value="{{ $id }}" {{ old('event_status_id') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
